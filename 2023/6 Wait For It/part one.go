...
func calculateMaxDistance(boat boat) int {
	...
	for chargeTheBoatTime < boat.Time {
		iteration := 0
		numberOfMilimetterPerMilisecond := 0
		for iteration < chargeTheBoatTime {
			numberOfMilimetterPerMilisecond++
			iteration++
		}
		distance := 0
		for iteration < boat.Time {
			distance += numberOfMilimetterPerMilisecond
			iteration++
		}
